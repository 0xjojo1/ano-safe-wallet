...
export function BaseNode({ accent = '#3b82f6', title, icon, actions, children, className }: BaseNodeProps) {
  return (
    <div className={cn('relative w-92 rounded-md border bg-card shadow-sm', className)}>
      {/* Accent bar without being affected by padding */}
      <div className=' inset-x-0 top-0 h-1.5 rounded-t-md' style={{ background: accent }}></div>
      {/* Content wrapper */}
      <div>
        {/* Ensure all elements are vertically centered for horizontal alignment */}
        <div className='px-3 py-2 flex items-center gap-3'>
          {icon ? <div className='h-8 w-8 rounded-full bg-muted flex items-center justify-center'>{icon}</div> : null}
